...
void setup() {
  ...
  Serial1.begin(9600);// to substitut for your gnss boudrate 
  configureGnss("0.5", true); //Serial port, Frequency, onlyGGA
...
void configureGnss(String freq, bool onlyGGA) {
  
  Serial1.flush();
  delay(50);
  sendUBX(setNavAir1G, sizeof(setNavAir1G)/sizeof(uint8_t)); // Set Airborne <1G Navigation Mode
  delay(50);
  sendUBX(setNmea4_1, sizeof(setNmea4_1)/sizeof(uint8_t)); // Set NMEA 4.1 (needed 4 Galileo)
  delay(50);
    if (freq == "1") {
      sendUBX(setFreq_1Hz, sizeof(setFreq_1Hz)/sizeof(uint8_t)); // Set GNSS to only GPS
      delay(50);
    }
  if (freq == "5") {
    sendUBX(setFreq_5Hz, sizeof(setFreq_5Hz)/sizeof(uint8_t)); // Set GNSS to only GPS
    delay(50);
  }
    if (freq == "0.5") {
    sendUBX(setFreq_05Hz, sizeof(setFreq_05Hz)/sizeof(uint8_t)); // Set GNSS to only GPS
    delay(50);
  }
  if (onlyGGA == true){
    sendUBX(setNmea_disableGRMC, sizeof(setNmea_enableGRMC)/sizeof(uint8_t));
    delay(50);
    sendUBX(setNmea_disableGSV, sizeof(setNmea_enableGSV)/sizeof(uint8_t));
    delay(50);
    sendUBX(setNmea_disableGLL, sizeof(setNmea_enableGLL)/sizeof(uint8_t));
    delay(50);
    sendUBX(setNmea_disableVTG, sizeof(setNmea_enableGSV)/sizeof(uint8_t));
    delay(50);
    sendUBX(setNmea_disableGGSA, sizeof(setNmea_enableGSV)/sizeof(uint8_t));
    //missing ublox messages to add the rest of NMEA sentences
    }

  if (onlyGGA == false){
    sendUBX(setNmea_enableGRMC, sizeof(setNmea_enableGRMC)/sizeof(uint8_t));
    delay(50);
    sendUBX(setNmea_enableGSV, sizeof(setNmea_enableGSV)/sizeof(uint8_t));
    delay(50);
    sendUBX(setNmea_enableGLL, sizeof(setNmea_enableGLL)/sizeof(uint8_t));
    delay(50);
    sendUBX(setNmea_enableVTG, sizeof(setNmea_enableGSV)/sizeof(uint8_t));
    delay(50);
    sendUBX(setNmea_enableGGSA, sizeof(setNmea_enableGSV)/sizeof(uint8_t));
    //missing ublox messages to add the rest of NMEA sentences
    }
